...
impl<F: TwoAdicField, Dft> ReedSolomonZkEncoding<F, Dft> {
    ...
    pub const fn new(t: usize, msg_len: usize, m: usize, dft: Dft) -> Self {
        assert!(m.is_power_of_two(), "Codeword length m must be a power of two");
        assert!(
            msg_len + t <= m,
...
mod tests {
    ...
    fn test_simulate_is_uniform_marginal_via_bijection() {
        ...
        let det_inv = det.try_inverse().expect("matrix at distinct positions is invertible");
        let r = vec![
